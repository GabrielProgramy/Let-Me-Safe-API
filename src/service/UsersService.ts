...
export default class UsersService {
	...
	async create(user: Users): Promise<Users> {
		...
		const years = Number(
			moment(user.birthDate, moment.HTML5_FMT.DATETIME_LOCAL)
				.fromNow()
				.match(/\d/g)
				.join(''),
		);
	...
	async updateUser(
		...
		if (address) {
			...
			if (!existsAddress) {
				const { cep, bairro, localidade, logradouro, complemento, uf } =
					await viaCepAPI(address);
	...
	async authenticateGoogle(oAuthtoken: string): Promise<string> {
		const { data } = await fetch(
			`https://www.googleapis.com/oauth2/v2/userinfo?oauth_token=${oAuthtoken}`,
		);
